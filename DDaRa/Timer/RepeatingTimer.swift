...
final class RepeatingTimer: SecondTimer {
    var timerState = TimerState.suspended
    var time = 0
    private var repeatingExecution: (() -> Void)?
    private var completion: (() -> Void)?
    var timers: (repeatTimer: DispatchSourceTimer?, nonRepeatTimer: DispatchSourceTimer?) = (DispatchSource.makeTimerSource(),DispatchSource.makeTimerSource())
